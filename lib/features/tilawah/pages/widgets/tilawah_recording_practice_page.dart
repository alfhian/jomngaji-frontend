...
class TilawahRecordingPrompt {
  final String arabicText;
  final String tip;

  const TilawahRecordingPrompt({required this.arabicText, required this.tip});
}

class TilawahRecordingPracticePage extends StatefulWidget {
  final String title;
  final String quizCode;
  final int lessonId;
  final Color accent;
  final String intro;
  final List<TilawahRecordingPrompt> prompts;

  const TilawahRecordingPracticePage({
    super.key,
    required this.title,
  ...
  @override
  State<TilawahRecordingPracticePage> createState() =>
      _TilawahRecordingPracticePageState();
}

class _TilawahRecordingPracticePageState
    extends State<TilawahRecordingPracticePage> {
  ...
  Future<void> _startRecording() async {
    ...
    final path =
        '${dir.path}/tilawah_${widget.quizCode}_${_index}_${DateTime.now().millisecondsSinceEpoch}.aac';
  ...
  Widget _buildPrompt(String text) {
    return RichText(
      ...
      text: TextSpan(
        ...
        children: [TextSpan(text: text, style: const TextStyle(color: Color(0xFF1F2937)))],
      ),
    );
  }
        ...
        child: ListView(
          ...
          children: [
            ...
            Container(
              ...
              child: _buildPrompt(prompt.arabicText),
            ),
            const SizedBox(height: 8),
            Text(
              prompt.tip,
