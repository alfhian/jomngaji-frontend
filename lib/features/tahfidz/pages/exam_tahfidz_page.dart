...
class ExamTahfidzPage extends StatefulWidget {
  const ExamTahfidzPage({super.key});

  @override
  State<ExamTahfidzPage> createState() => _ExamTahfidzPageState();
}
...
class _ExamTahfidzPageState extends State<ExamTahfidzPage> {
  ...
  final List<_ExamQuestion> _questions = const [
    _ExamQuestion.mcq(
      prompt: 'Pilih metode tahfidz yang paling tepat',
      arabic: 'إِنَّا أَعْطَيْنَاكَ الْكَوْثَرَ',
      options: ['Ulangi ayat per potong', 'Hafal sekaligus tanpa ulang', 'Cukup dibaca sekali'],
      correct: 'Ulangi ayat per potong',
    ),
    ...
    _ExamQuestion.mcq(
      prompt: 'Apa yang perlu dilakukan setelah setoran hafalan?',
      arabic: 'فَصَلِّ لِرَبِّكَ وَانْحَرْ',
      options: ['Muraja’ah hafalan lama', 'Langsung ganti surat', 'Tidak perlu diulang'],
      correct: 'Muraja’ah hafalan lama',
    ),
    _ExamQuestion.pronunciation(
      prompt: 'Latihan pengucapan',
      arabic: 'فَصَلِّ لِرَبِّكَ وَانْحَرْ',
    ...
    _ExamQuestion.mcq(
      prompt: 'Target hafalan menengah yang paling tepat adalah...',
      arabic: 'إِنَّ شَانِئَكَ هُوَ الْأَبْتَرُ',
      options: ['Sambung antar ayat dengan lancar', 'Tambah halaman secepatnya', 'Fokus jumlah saja'],
      correct: 'Sambung antar ayat dengan lancar',
    ),
    _ExamQuestion.pronunciation(
      prompt: 'Latihan pengucapan',
      arabic: 'إِنَّ شَانِئَكَ هُوَ الْأَبْتَرُ',
      targetPronunciation: 'إِنَّ شَانِئَكَ هُوَ الْأَبْتَرُ',
      lessonId: 2,
    ),
    _ExamQuestion.mcq(
      prompt: 'Sikap terbaik saat setoran hafalan adalah...',
      arabic: 'قُلْ هُوَ اللَّهُ أَحَدٌ',
      options: ['Tenang dan konsisten', 'Tergesa agar cepat selesai', 'Meloncat ayat saat ragu'],
      correct: 'Tenang dan konsisten',
    ),
    _ExamQuestion.pronunciation(
      prompt: 'Latihan pengucapan',
      arabic: 'قُلْ هُوَ اللَّهُ أَحَدٌ',
      targetPronunciation: 'قُلْ هُوَ اللَّهُ أَحَدٌ',
      lessonId: 3,
    ),
    _ExamQuestion.mcq(
      prompt: 'Tujuan muraja’ah pada level mahir adalah...',
      arabic: 'اللَّهُ الصَّمَدُ',
      options: ['Menjaga hafalan tetap kuat', 'Menambah hafalan baru saja', 'Mengganti urutan ayat'],
      correct: 'Menjaga hafalan tetap kuat',
    ),
    _ExamQuestion.pronunciation(
      prompt: 'Latihan pengucapan',
      arabic: 'اللَّهُ الصَّمَدُ',
      targetPronunciation: 'اللَّهُ الصَّمَدُ',
      lessonId: 3,
    ),
  ];
  ...
  Future<void> _startRecording() async {
    ...
    final path = '${dir.path}/exam_tahfidz_${DateTime.now().millisecondsSinceEpoch}.aac';
  ...
  Future<void> _evaluatePronunciation() async {
    ...
    try {
      final json = await _api.evaluateTahfidz(
  ...
  Future<void> _submitTahfidzExam({
    ...
    final res = await http.post(
      Uri.parse('$_baseUrl/tahfidz-exam/submit'),
      headers: headers,
      body: jsonEncode({
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CustomGradientAppBar(title: 'Tes Akhir Tahfidz'),
      body: Stack(
        children: [
          Positioned.fill(
            child: Image.asset(
