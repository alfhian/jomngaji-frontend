...
class LatihanSukuKataLevel8Page extends StatefulWidget {
  const LatihanSukuKataLevel8Page({super.key});

  @override
  State<LatihanSukuKataLevel8Page> createState() =>
      _LatihanSukuKataLevel8PageState();
}

class _LatihanSukuKataLevel8PageState
    extends State<LatihanSukuKataLevel8Page> with TickerProviderStateMixin {

  // ===============================
  // DATA TASDYDID
  // ===============================
  final List<Map<String, String>> level8Words = [
    {"ar": "مُحَمَّد", "lat": "MUHAMMAD"},
    {"ar": "رَبِّ", "lat": "RABBI"},
    {"ar": "بَسَّام", "lat": "BASSAM"},
    {"ar": "حَدَّثَ", "lat": "HADDATSA"},
    {"ar": "مَدَّ", "lat": "MADDA"},
    {"ar": "عَمَّ", "lat": "AMMA"},
    {"ar": "قَوَّى", "lat": "QAWWA"},
    {"ar": "شَدَّ", "lat": "SHADDA"},
    {"ar": "غَلَّقَ", "lat": "GHALLAQA"},
    {"ar": "حَيَّة", "lat": "HAYYAH"},
  ];
  ...
  void generateNewQuestion() {
    ...
    question = level8Words[random.nextInt(level8Words.length)];
    ...
    while (options.length < 3) {
      final randomOpt =
          level8Words[random.nextInt(level8Words.length)]["lat"]!;
      if (!options.contains(randomOpt)) {
  ...
  void finishLevel() async {
    ...
    await SukuKataService.saveLevelScore(8, score);

    if (score >= 50) {
      final prefs = await SharedPreferences.getInstance();
      prefs.setBool("level_9_unlocked", true);
    }
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              score >= 50 ? "MasyaAllah! Hebat!" : "Yuk Coba Lagi!",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Latihan Level 8", style: GoogleFonts.poppins()),
